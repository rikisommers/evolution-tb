...
const migration = (argv) => __awaiter(void 0, void 0, void 0, function* () {
    const { context } = argv;
    const { managementToken, activeSpaceId, activeEnvironmentId, proxy, rawProxy, host } = context;
    const managementApplication = `contentful.cli/${version}`;
    const managementFeature = `space-migration`;
    const options = Object.assign(Object.assign({}, argv), { spaceId: activeSpaceId, managementApplication,
        managementFeature, accessToken: managementToken, environmentId: activeEnvironmentId, host: host, headers: getHeadersFromOption(argv.header) });
    if (proxy) {
