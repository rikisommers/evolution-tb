...
const { codeStyle, pathStyle, headingStyle, blockQuoteStyle, htmlStyle, pathTextStyle } = require('./styles');
function CustomRenderer() {
    ...
    renderer.heading = (text, level) => `${level === 1 ? '\n' : ''}${headingStyle(text)}\n\n`;
    renderer.paragraph = text => `${text}\n\n`;
    renderer.blockquote = code => `${blockQuoteStyle(code)}\n`;
    renderer.html = arg => `${htmlStyle(arg)}\n`;
    renderer.hr = () => `${chalk.underline(Array(process.stdout.columns).join('_'))}\n\n`;
...
module.exports = string => marked(sanitizeString(string), {
